"""
Server startup module for Lumen VLM Service.
...
from lumen_vlm.fastvlm.fastvlm_service import GeneralFastVLMService
from lumen_vlm.proto import ml_service_pb2_grpc as pb_rpc
...
def serve(config_path: str, port_override: int | None = None) -> None:
    ...
    try:
        ...
        service_config = config.services.get("vlm")
...
Examples:
  ...
  python server.py --config config/vlm.yaml

  # Validate config without starting server
  lumen-resources validate config/vlm.yaml
        """,
    )
