...
lumen-app 是 Lumen 项目的本地控制平面与 Web 管理界面，负责服务安装、配置、启动与监控，并通过 FastAPI 提供 HTTP/WebSocket API，静态托管 web-ui。作为应用层项目，它不遵循 ML 服务模块的三层架构契约，但必须遵守以下规范。
...
- **控制平面 API**：提供 FastAPI HTTP/WebSocket 接口给前端调用
- **Web UI**：提供基于 React/Vite 的管理界面（web-ui）
- **配置管理**：管理设备配置、服务配置、环境配置
- **依赖管理**：处理 Python 依赖、系统依赖的安装和解析
- **服务发现**：在配置层支持 mDNS 元数据，预留本地服务发现能力

**不负责**：
- 不实现任何 ML 推理逻辑
- 不直接处理模型加载和执行
- 不实现 ML gRPC 服务端（仅负责拉起与管理独立 ML 服务进程）
...
**禁止使用的技术**：
- 禁止引入桌面 GUI 框架作为主要界面（如 PyQt、Tkinter、Flet）
- 禁止直接管理 ML 模型文件
...
│   └── static/              # 前端构建产物（web-ui dist）
└── web-ui/                  # React/Vite 前端工程
```

### II. 分层架构（应用层）

lumen-app 采用前后端分离的三层结构：Web UI（web-ui）→ API（FastAPI）→ Services/Utils（控制平面）。

#### 1. Web UI 层（Presentation Layer）

**职责**：渲染界面、处理用户交互、展示状态

**组件**：
- **Pages**：独立的功能页面（Installer、Runner、Monitor）
- **Components**：可复用的 UI 组件（按钮、卡片、表单）
- **API Client**：封装对 FastAPI 的请求与响应处理

**契约规范**：
```python
@requires:
  - 必须使用 web-ui（React/Vite）作为主界面
  - 必须通过 HTTP/WebSocket 访问后端 API
  - 必须实现响应式布局
  - 必须提供 loading 状态反馈
...
@prohibitions:
  - 禁止在 Web UI 直接调用 gRPC 服务
  - 禁止在 Web UI 直接修改服务配置文件
```
...
#### 2. API/Services 层（Business Logic Layer）

**职责**：业务逻辑、状态管理、服务编排与 API 暴露

**模块**：
- **API Routers**：HTTP/WS 路由（config/install/server/logs）
- **Schemas**：请求/响应模型
- **Services**：配置生成、安装、服务进程管理、应用状态
- **State**：统一应用状态（app_state）

**契约规范**：
```python
@requires:
  - 必须使用 lumen-resources 的 LumenConfig
  - 必须通过服务管理模块控制 ML 服务进程
  - 必须管理应用状态（State）
...
#### 4. Integration 层（通信与外部交互）

**职责**：与外部 ML 服务/系统交互的协议与客户端封装

**内容**：
- HTTP/WS 客户端（如健康检查、日志流）
- 可选的 gRPC 客户端封装（若引入）
...
@prohibitions:
  - 禁止在 Web UI 直接进行低层通信
  - 禁止绕过 API/Services 直接触达外部服务
```

### III. 数据流原则

**单向数据流**：Web UI → API → Services/Integration → ML Services
...
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
│  Integration Layer (HTTP/WS/gRPC Clients)                   │
│  - 日志流/健康检查/外部通信封装                              │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
...
**数据流规则**：
- Web UI 只能通过 API 层访问服务
- API 层只能通过 Services/Integration 与外部服务通信
- 禁止反向依赖（Services 不依赖 Web UI 实现）
- 状态更新必须通过 WebSocket 或轮询机制通知前端
...
```
~/.lumen/
├── lumen-config.yaml       # 生成的服务配置
├── cache/                  # 缓存目录
├── logs/                   # 日志目录
├── micromamba/             # micromamba 安装与环境
└── wheels/                 # 依赖包缓存
```
...
1. 检测设备和环境
2. 解析依赖关系
3. 安装 Python 包（micromamba 环境内 pip）
4. 配置服务参数
5. 验证安装结果
