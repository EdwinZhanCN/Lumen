...
DEVICE_PRESETS: Dict[str, Optional[DeviceConfig]] = {
    "Apple Silicon (Apple M)": DeviceConfig.apple_silicon(),
    ...
    "AMD iGPU (Radeon)": DeviceConfig.amd_gpu_win(),
    "AMD NPU": DeviceConfig.amd_npu(),
...
class DeviceConfView(ft.Column):
            ...
            content = ft.Column(
                [
                    ...
                    self._build_info_row(
                        ...
                        self._format_onnx_providers(config.onnx_providers),
                    ),
                    self._build_info_row(
    ...
    def _format_onnx_providers(self, providers: list | None) -> str:
        """
        Format ONNX providers for display

        Args:
            providers: List of providers (can be strings or tuples)
        ...
        for provider in providers:
            if isinstance(provider, tuple):
                # Tuple format: (provider_name, config_dict)
