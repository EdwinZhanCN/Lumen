...
# from ..i18n_manager import t is not used
from .advanced_view import create_advanced_view
from .device_conf_view import create_device_conf_view
from .installer_view import create_installer_view
from .presets_view import create_presets_view
from .welcome_view import create_welcome_view
...
class RunnerView:
    ...
    def create_view(self) -> ft.Container:
        """创建并返回主视图容器"""
        # 使用当前状态初始化
        self.update_view(self.state_machine.current_state)
        return self.main_container

    def update_view(self, state: RunnerState):
        """
        基于当前状态更新视图
    ...
    def _create_welcome_view(self):
        """创建欢迎视图"""
        welcome_view = create_welcome_view(
            lumilio_handler=lambda e: self.state_machine.on_welcome_lumilio_photos(),
    ...
    def _create_device_conf_view(self):
        ...
        device_conf_view = create_device_conf_view(
    ...
    def _create_presets_view(self):
        ...
        # 创建预设视图
        presets_view = create_presets_view(
            button_container=button_container, data_binding=data_binding
        )
    ...
    def _create_advanced_view(self):
        ...
        # 创建高级视图
        advanced_view = create_advanced_view(
            button_container=button_container, data_binding=data_binding
        )
    ...
    def _create_installer_view(self):
        """创建安装器视图"""
        # 创建按钮组件
        continue_btn = ContinueButton("完成安装")
        reset_btn = ResetButton("返回")
        ...
        # 创建安装器视图
        installer_view = create_installer_view(
            button_container=button_container, data_binding=data_binding
        )
    ...
    def _create_active_view(self):
        """创建活动/运行状态视图"""
        # 创建按钮组件
        continue_btn = ContinueButton("查看日志")
        reset_btn = ResetButton("停止")
        ...
        # 绑定自定义处理程序
        continue_btn.get_button().on_click = lambda e: print("View logs")
        reset_btn.get_button().on_click = (
            lambda e: self.state_machine.on_reset_to_welcome()
        )
        ...
        # 导入并使用活动运行器视图
        from .active_runner_view import create_active_runner_view

        # 创建活动视图
        active_view = create_active_runner_view(
            button_container=button_container, data_binding=data_binding
        )
